...
LATEST (2026-09-19, orbit/journey pass): The visual system now extends to a CSS-only 3D-like orbital field on the desktop landing hero, a responsive three-stop journey index, coordinate-grid map treatment, and more legible selected form/result states. Inspo archive references were `wesbos-com`, `norrona-com`, and `frontierclimate-com`; their composition informed the work, while Hail Mary’s Liter / paper / terracotta system remains intact. `npm.cmd run build` passed; desktop and 390px browser checks show no horizontal overflow or console errors. No commit, push, or deployment was made in this pass.
